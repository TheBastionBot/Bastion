  ...
  cod4.query({
    ...
    port: port
      ...
      if (data.players.length > 0) {
        let players = [];
        let scores = [];
        for (i = 0; i < stats.players.length; i++) {
          players.push(stats.players[i].name);
        }
        for (i = 0; i < stats.players.length; i++) {
          scores.push(stats.players[i].frags);
        }
        stats.push(
          {
            name: 'Player',
      ...
      message.channel.send({embed: {
        ...
        title: data.name,
        description: 'Call of Duty 4: Modern Warfare',
        fields: stats,
        footer: {
...
exports.help = {
  ...
  botPermission: '',
  userPermission: '',
  usage: 'cod4 <COD_SERVER_IP>[:PORT]',
  example: ['cod4 139.59.31.128', 'cod4 139.59.31.128:27016']
};
