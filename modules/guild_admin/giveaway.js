  ...
  try {
    if (args.item) {
      ...
      if (args.timeout < 1 || args.timeout > 24) {
        return Bastion.emit('error', Bastion.strings.error(message.guild.language, 'invalidInput'), 'Giveaway can only run for at lease an hour and at most 24 hours.', message.channel);
      }
