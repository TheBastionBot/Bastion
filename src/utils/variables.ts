...
export const replace = (string: string, context: PartialGuildMember | GuildMember | Message): string => {
    const vars: { [key: string]: string | number } = {
        ...
        "{server.users.size}": context?.guild.members.cache.filter(context => context?.user.bot === false).size,    // based on member cache
        "{server.bots.size}": context?.guild.members.cache.filter(context => context?.user.bot === true).size,      // based on member cache
        ...
        "{author.nick}": context instanceof Message ? (context?.member?.displayName ?? context?.author.displayName) : context?.displayName,
        "{author.avatar}": context instanceof Message ? context?.author.displayAvatarURL() : context?.user.displayAvatarURL(),
        "{author.roles.size}": context instanceof Message ? (context?.member?.roles.cache.size ?? 0) : context?.roles.cache.size,
